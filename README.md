...
Use `./bin/omarchy-iso-make --locale ja` for Japanese input and user locale defaults. Add `--with-dictation` to include offline Japanese dictation. See [locale profiles](locales/README.md) for the build options and scope, and the [Japanese profile](locales/ja/README.md) for the earlier downloadable prototype.
