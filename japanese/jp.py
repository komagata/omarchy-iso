...
def configure_jp(ctx):
    if not ctx.username:
        raise RuntimeError('Japanese defaults require an installation user; deferred provisioning is not supported.')
    ...
    subprocess.run(['arch-chroot', str(ctx.target), 'runuser', '-u', ctx.username, '--', 'python', '/usr/local/share/omarchy-jp/jp_defaults.py', '--user'], check=True)
